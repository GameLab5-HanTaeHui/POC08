...
namespace SEAL
{
    ...
    public class BossPattern_RageCharge : BossPatternBase
    {
        ...
        private BossWardenPatternDataSO.RageChargeSettings RageChargeData
            => _data != null && _data.PatternData != null
                ? _data.PatternData.RageCharge
                : BossWardenPatternDataSO.DefaultRageCharge;
        ...
        {
            ConfigureLifecycle(RageChargeData.lifecycle, CommonData);
            _triggerGroggyOnRecovery = false;
        }
        ...
        protected override IEnumerator OnWarning()
        {
            ...
            var pattern = RageChargeData;

            // ① 3방향 계산: 정면 / 좌/우 sideAngle
            Vector2 baseDir = _ai.FacingDir;
            _chargeDirections[0] = baseDir;
            _chargeDirections[1] = Quaternion.Euler(0f, 0f, pattern.sideAngle) * baseDir;
            _chargeDirections[2] = Quaternion.Euler(0f, 0f, -pattern.sideAngle) * baseDir;

            // ② 양팔 백스윙: 기본 방향(baseDir) 반대로 당기기
            Vector3 backswingOffset = new Vector3(
                -baseDir.x * pattern.windupPullAmount,
                -baseDir.y * pattern.windupPullAmount,
                0f);
            ...
            float lineLength = pattern.WarningLineLength;
        ...
        protected override IEnumerator OnActive()
        {
            ...
            var pattern = RageChargeData;
            int chargeCount = Mathf.Min(pattern.count, _chargeDirections.Length);
            ...
            for (int i = 0; i < chargeCount; i++)
            {
                ...
                Vector3 thrustOffset = new Vector3(
                    dir.x * pattern.thrustAmount,
                    dir.y * pattern.thrustAmount,
                    0f);
                ...
                bool hitWall = false;
                float elapsed = 0f;
                float maxDuration = (pattern.distance / Mathf.Max(speed, 0.1f)) * 1.5f;
                ...
                while (true)
                {
                    ...
                    if (!hasHit)
                    {
                        Collider2D hit = Physics2D.OverlapBox(
                            _rigid2D.position + dir * (pattern.hitboxSize.y * 0.5f),
                            pattern.hitboxSize,
                ...
                if (i < chargeCount - 1)
                    yield return StartCoroutine(WaitForPattern(pattern.interval));
            }
