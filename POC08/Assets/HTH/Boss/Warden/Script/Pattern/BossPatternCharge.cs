...
namespace SEAL
{
    ...
    public class BossPattern_Charge : BossPatternBase
    {
        ...
        private BossWardenPatternDataSO.ChargeSettings ChargeData
            => _data != null && _data.PatternData != null
                ? _data.PatternData.Charge
                : BossWardenPatternDataSO.DefaultCharge;
        ...
        {
            ConfigureLifecycle(ChargeData.lifecycle, CommonData);
        }
        ...
        protected override IEnumerator OnWarning()
        {
            ...
            _attackRange?.ShowChargeLine(
                bossPos,
                _chargeDirection,
                pattern.warningSize.y,
                pattern.warningSize.x);
            ...
            if (_armRTransform != null)
            {
                ...
                _armRTransform
                    .DOLocalMove(_armOriginLocalPos + localBackDir * pattern.windupPullAmount,
        ...
        protected override IEnumerator OnActive()
        {
            ...
            var pattern = ChargeData;
            float chargeSpeed = pattern.GetSpeed(_isPhase2);
            float distance = pattern.distance;
            ...
            if (_armRTransform != null)
            {
                ...
                _armRTransform
                    .DOLocalMove(_armOriginLocalPos + localDir * pattern.thrustAmount, 0.1f)
                    .SetEase(Ease.OutExpo);
            }

            // 돌진
            if (_rigid2D != null)
                _rigid2D.linearVelocity = _chargeDirection * chargeSpeed;

            _chargeStartPos = _rigid2D != null ? _rigid2D.position : (Vector2)transform.position;
            float elapsed = 0f;
            float maxDuration = distance / Mathf.Max(chargeSpeed, 0.1f) * 2f;
            int logFrame = 0;
            ...
            while (elapsed < maxDuration)
            {
                ...
                float currentSpeed = _rigid2D != null ? _rigid2D.linearVelocity.magnitude : 0f;

                if (logFrame++ % 30 == 0)
                    Debug.Log($"[BossPattern_Charge] 진행 | 거리:{dist:F2}/{distance} 속도:{currentSpeed:F2}");
        ...
        private void CheckChargeHit()
        {
            ...
            int count = Physics2D.OverlapBoxNonAlloc(boxCenter, pattern.hitboxSize, angle, _hitResults, _playerLayer);
