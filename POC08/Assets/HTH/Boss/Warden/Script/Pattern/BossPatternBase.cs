...
//
// [수정 내용]
//   v2.0 — PatternDataSO 생애주기 수치 주입 지원
...
namespace SEAL
{
    ...
    public abstract class BossPatternBase : MonoBehaviour
    {
        ...
        /// <summary>
        /// BossWardenDataSO 주입.
        /// 하위 패턴이 필요하면 override 해서 자기 _data 에 저장하고
        /// PatternDataSO 의 개별 패턴 설정을 Apply 한다.
        /// </summary>
        public virtual void Initialize(BossWardenDataSO data)
        {
        }

        /// <summary>
        /// PatternDataSO 의 공통/개별 생애주기 설정을 기존 Base 필드에 반영한다.
        /// 기존 ExecuteWarning / ExecuteRecovery 흐름은 그대로 유지하면서
        /// 수치 출처만 PatternDataSO 로 옮기기 위한 Step 2 브리지다.
        /// </summary>
        protected void ConfigureLifecycle(
            BossWardenPatternDataSO.LifecycleSettings lifecycle,
            BossWardenPatternDataSO.CommonSettings common)
        {
            if (lifecycle == null) return;

            _cooldown = lifecycle.GetCooldown(common);
            _warningDuration = lifecycle.GetWarningDuration(common);
            _recoveryDuration = lifecycle.GetRecoveryDuration(common);
            _isPhase2Only = lifecycle.phase2Only;
        }
