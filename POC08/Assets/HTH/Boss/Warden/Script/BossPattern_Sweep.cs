...
namespace SEAL
{
    ...
    public class BossPattern_Sweep : BossPatternBase
    {
        ...
        /// <summary>Warning 시 팔이 좌우로 벌어지는 거리 (로컬 오프셋).</summary>
        [Tooltip("팔 벌리기 거리. 권장: 0.6")]
        [Min(0f)]
        [SerializeField] private float _armSpreadAmount = 0.6f;

        /// <summary>회전 완료 후 팔이 날아가는 거리. 2페이즈에서는 × 1.5 적용.</summary>
        ...
        protected override IEnumerator OnActive()
        {
            ...
            if (_armLTransform != null)
            {
                ...
                _armLTransform
                    .DORotate(new Vector3(0f, 0f, armLAngle), _flyDuration * 0.5f)
            ...
            if (_armRTransform != null)
            {
                ...
                _armRTransform
                    .DORotate(new Vector3(0f, 0f, armRAngle), _flyDuration * 0.5f)
                    .SetEase(Ease.OutQuart);
            }
