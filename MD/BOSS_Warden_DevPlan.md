...
| 파일 | 작업 내용 | 상태 |
|---|---|---|
| `BossWardenAI.cs` | Idle/Chase 상태 + 플레이어 추적 이동 | ⬜ 미구현 |
| `BossWardenAI.cs` | Warning/Active/Recovery 패턴 실행 흐름 | ⬜ 미구현 |
| `BossWardenAI.cs` | _isStopped 플래그 (그로기/딜페이즈 중 정지) | ⬜ 미구현 |
| `BossWardenFeedback.cs` | 상태별 색상 전환 (Idle/Chase/Warning/Active/Recovery) | ⬜ 미구현 |
